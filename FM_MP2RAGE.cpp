...
//
//     Project: NUMARIS/4
//        File: \n4_servers1\pkg\MrServers\MrImaging\seq\fm_radial\fm_radial.cpp
...
///         - fm_radial
///
/// This file contains the implementation of the class fm_radial.
/// The sequence fm_radial and Flash1 use it to generate very nice images.
///
//  **************************************************************************
...
#ifdef SEQUENCE_CLASS_fm_radial
SEQIF_DEFINE (SEQ_NAMESPACE::fm_radial)
#endif
...
//. ----------------------------------------------------------
//. Instantiate fm_radial ui parameter and Gradient tables (ParameterMap)
//. ----------------------------------------------------------
...
//    will dump "fm_radial.cpp:  This is my debug text with TR: 1000" to the standard output, if
...
NLSStatus fm_radial::initialize (SeqLim &rSeqLim)
{
	...
	"() >>>> Welcome to the clean fm_radial sequence  (Build: "__DATE__"   "__TIME__")") ;
	...
	if ( (NLS_SEV & (lStatus = createUI (rSeqLim))) == NLS_SEV )  {
		TRACE_PUT1(TC_ALWAYS, TF_SEQ,"%s: Instantiation of UI class failed: fm_radial::createUI(SeqLim&)", ptModule);
		return ( lStatus );
	}
...
//
//  --------------------------------------------------------------------------
NLSStatus fm_radial::prepare (MrProt &rMrProt, SeqLim &rSeqLim, SeqExpo &rSeqExpo)
{
	//. -----------------------------------------------------------------------------
	//. Local variables
	//. -----------------------------------------------------------------------------
...
if (rMrProt.gradSpec().isGSWDMode()) m_dMinRiseTime =  rMrProt.gradSpec().GSWDMinRiseTime();
	...
	MinDurationBetweenRFandADC = 40;// fm_radial WIP SIEMENS
...
//
//  --------------------------------------------------------------------------
NLSStatus fm_radial::check (MrProt  &rMrProt, SeqLim &rSeqLim, SeqExpo &rSeqExpo, SEQCheckMode *pSEQCheckMode)
{
...
//
//  --------------------------------------------------------------------------
NLSStatus fm_radial::run (MrProt &rMrProt, SeqLim &rSeqLim, SeqExpo &rSeqExpo)
{
	//. --------------------------------------------------------
	//. Local variables
	//. --------------------------------------------------------
	...
	MaxProjectionInPhase=u_Projections;
		...
		sprintf(ptFilename,"fm_radialPMUsignal_%s",sDateTime);
...
//
//   --------------------------------------------------------------------------
NLS_STATUS fm_radial::runKernel(MrProt &rMrProt,SeqLim &rSeqLim, SeqExpo &rSeqExpo, long lKernelMode, long ProjectionNumber,long lChronologicSlice, long lpartition )
{
	//. --------------------------------------------------------------------------
	//. Local variables
	//. --------------------------------------------------------------------------
...
NLS_STATUS fm_radial::createUI (SeqLim&)
{
...
}   // end:fm_radial::createUI



const fm_radialUI* fm_radial::getUI (void) const
{
	return ( m_pUI );
}
...
long   fm_radial::getKernelRequestsPerMeasurement(void)
{
...
long   fm_radial::getKernelCallsPerRelevantSignal(void)
{
	return (m_lKernelCallsPerRelevantSignal);

}

long   fm_radial::getDurationMainEventBlock(void)
{
	return (m_lDurationMainEventBlock);

}

long   fm_radial::getScanTimeAllSats(void)
{
	return (m_lScanTimeAllSats);

}

double fm_radial::getdMinRiseTime(void)
{
	return (m_dMinRiseTime);

}

double fm_radial::getdGradMaxAmpl(void)
{
